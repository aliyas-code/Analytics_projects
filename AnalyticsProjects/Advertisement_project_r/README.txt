
Description

(ENG)
The visitors of the Internet site who visited it through advertising were investigated. This advertisement worked quite badly, not a single targeted action was achived. So it was decided to make  an analysis of site users who went through ads to the site to detect problems and gain knowledge about the audience for further reconfiguration of ads.
Information about more than 1000 visits were extracted using the Yandex Logs API. Data contains:
-        visitID;
-        isNewUser;
-        clientID;
-        date;
-        bounce;
-        AdType (advertisement type);
-        BannerGroupId;
-        BannerGroupName;
-        BannerId;
-        BannerName;
-        startURL;
-        lastPlatform;
-        Condition (advertising display conditions);
-        deviceCategory;
-        operatingSystem;
-        PageViews;
-        visitDuration;
-        price (price of visit);
-        WatchCount (the number of views that were in this visit).
=====================================================================================================

(RUS)

Исследуются посетители сайта, перешедшие на него по рекламе. Данная реклама сработала достаточно плохо, не было выполнено ни одного целевого действия. Так что было решено провести анализ пользователей сайта, перешедших по рекламе, для обнаружения проблем и получения знаний об аудитории для дальнейшей перенастройки рекламы. 
Была получена информация о более 1000 визитов. Информация о визитах была получена с помощью Yandex Logs API. Информация содержит данные: 
-        visitID (Идентификатор визита);
-        isNewUser (Первый ли визит посетителя);
-        clientID (Идентификатор клиента);
-        date (Дата визита);
-        bounce (Отказность);
-        AdType (Тип рекламы);
-        BannerGroupId (Идентификатор группы баннеров);
-        BannerGroupName (Имя группы баннеров);
-        BannerId (Идентификатор баннера);
-        BannerName (Имя баннера);
-        startURL (Страница входа);
-        lastPlatform (Источник трафика);
-        Condition (Условие показа объявления);
-        deviceCategory (Тип устройства);
-        operatingSystem (Операционная система);
-        PageViews (Количество просмотренных страниц);
-        visitDuration (Продолжительность визита);
-        price (Цена визита);
-        WatchCount (Количество просмотров, которые были в данном визите).
